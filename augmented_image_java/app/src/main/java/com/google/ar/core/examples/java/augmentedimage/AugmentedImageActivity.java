...
* Activity this functionality is render augmented image into "default.jpg|png" image
* 14/05/2019
...
public class AugmentedImageActivity extends AppCompatActivity implements GLSurfaceView.Renderer 
{
	...
	**/
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		this.glideRequestManager.load(Uri.parse("file:///android_asset/fit_to_scan.png")).into(fitToScanView);
	...
	**/
	...
	protected void onResume() 
	{
		...
		if(this.session == null) 
		{
			...
			try 
			{
				switch(ArCoreApk.getInstance().requestInstall(this, !this.installRequested)) 
				{
			...
			if(message != null) 
			{
				this.messageSnackbarHelper.showError(this, message);
	...
	/**
	* When a user want exit of the activity, the system call this method like first instance
	*
	*
	*
	*
	**/
	...
	private void configureSession() 
	{
		...
		if(!this.setupAugmentedImageDatabase(config)) 
		{
			this.messageSnackbarHelper.showError(this, "Could not setup augmented image database");
		}
	...
	private void drawAugmentedImages(Frame frame, float[] projmtx, float[] viewmtx, float[] colorCorrectionRgba) 
	{
		...
		for(AugmentedImage augmentedImage : updatedAugmentedImages) 
		{
			switch(augmentedImage.getTrackingState()) 
			{
				...
				case TRACKING:
					...
					if(!this.augmentedImageMap.containsKey(this.augmentedImage.getIndex())) 
					{
						Anchor centerPoseAnchor = augmentedImage.createAnchor(augmentedImage.getCenterPose());
						this.augmentedImageMap.put(augmentedImage.getIndex(), Pair.create(augmentedImage, centerPoseAnchor));
					}
					break;
		...
		for(Pair<AugmentedImage, Anchor> pair : augmentedImageMap.values()) 
		{
			...
			{
				case TRACKING:
					this.augmentedImageRenderer.draw(viewmtx, projmtx, augmentedImage, centerAnchor, colorCorrectionRgba);
					break;
				default:
					break;
			}
