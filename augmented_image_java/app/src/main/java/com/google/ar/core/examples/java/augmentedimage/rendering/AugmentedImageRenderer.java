...
/** 
* @autor: Alejandro Lindarte Castro
*
* This class render agmented images
*
*  
*
*/
public class AugmentedImageRenderer 
{
	...
	public AugmentedImageRenderer() {}//constructor

	/**
	* in charge of create one thread for OPENGL
	* 
	*
	*
	**/
	public void createOnGlThread(Context context) throws IOException 
	{
		...
		this.imageFrameUpperLeft.setBlendMode(BlendMode.SourceAlpha);
		...
		this.imageFrameUpperRight.setBlendMode(BlendMode.SourceAlpha);
		...
		this.imageFrameLowerLeft.setBlendMode(BlendMode.SourceAlpha);
		...
		this.imageFrameLowerRight.setBlendMode(BlendMode.SourceAlpha);
		
	}//createOnGlThread

	/**
	*
	* draw on a "default" picture
	*
	*
	*
	**/
	public void draw(float[] viewMatrix, float[] projectionMatrix, AugmentedImage augmentedImage, Anchor centerAnchor, float[] colorCorrectionRgba) 
	{
		float[] tintColor = convertHexToColor(TINT_COLORS_HEX[augmentedImage.getIndex() % TINT_COLORS_HEX.length]);

		Pose[] localBoundaryPoses = 
		{
			Pose.makeTranslation(-0.5f * augmentedImage.getExtentX(),0.0f,-0.5f * augmentedImage.getExtentZ()), // upper left
			Pose.makeTranslation(0.5f * augmentedImage.getExtentX(),0.0f,-0.5f * augmentedImage.getExtentZ()), // upper right
			Pose.makeTranslation(0.5f * augmentedImage.getExtentX(),0.0f,0.5f * augmentedImage.getExtentZ()), // lower right
			Pose.makeTranslation(-0.5f * augmentedImage.getExtentX(),0.0f,0.5f * augmentedImage.getExtentZ()) // lower left
		};
		...
		this.imageFrameUpperLeft.updateModelMatrix(modelMatrix, scaleFactor);
		this.imageFrameUpperLeft.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);

		worldBoundaryPoses[1].toMatrix(modelMatrix, 0);
		this.imageFrameUpperRight.updateModelMatrix(modelMatrix, scaleFactor);
		this.imageFrameUpperRight.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);

		worldBoundaryPoses[2].toMatrix(modelMatrix, 0);
		this.imageFrameLowerRight.updateModelMatrix(modelMatrix, scaleFactor);
		this.imageFrameLowerRight.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);

		worldBoundaryPoses[3].toMatrix(modelMatrix, 0);
		this.imageFrameLowerLeft.updateModelMatrix(modelMatrix, scaleFactor);
		this.imageFrameLowerLeft.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);
		
	}//draw

	/**
	* In charge to convert a int primitival to float array of colors (RedGreenBlue)
	*
	*
	*
	**/
